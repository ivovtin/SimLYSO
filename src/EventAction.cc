...
EventAction::EventAction(PrimaryGeneratorAction* generator, RunAction* action, int _idVerbose):ScHitsCollID(-1), EnHitsCollID(-1), NumHitsCollID(-1), primary(generator), runAction(action)
{
  verbose    = false;
        verboseTop = false;
...
//		std::vector<int> vTime;
		...
		for (G4int iHit = 0; iHit<nOptHits; iHit++)
		{
			...
			//if (creator == "Cerenkov") { man->FillH1(man->GetH1Id("cht"), nTime+dt, 1); tsp->Fill(nTime+dt);  
			//man->FillH1(man->GetH1Id("pht"), nTime, 1);
